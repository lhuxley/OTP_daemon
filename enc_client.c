...
/*****************************************************************************************************************************
 * This function is used to open the plaintext file specified by the user in argv[1] and store it's data into the plaintext variable
 * parameters: plaintext variable to store the contents of the file, name of the plaintext file
 * **************************************************************************************************************************/
void store_plaintext_content(char* plaintext, char* plaintext_file_name){

    FILE * plaintext_file = fopen(plaintext_file_name, "r");

    if(plaintext_file == NULL)
        error("The plaintext file specified could not be opened!\n", 1);

    fgets(plaintext, 255, (FILE*)plaintext_file); // store name of plaintext
...
/*****************************************************************************************************************************
 * This function is used to combine the contents of the plaintext and the key variables into one long string.
 * Then new variable created by this, plaintext_key looks like this: "contentsofkeyvariable|contentsofplaintextvariable"
 * This is done so that the contents of the key and plaintext files can be sent only using one send(). 
 * 
 * parameters: variable to store newly created key|plaintext variable, the contents of the key file, the contents of the plaintext file
 * **************************************************************************************************************************/
void combine_key_plaintext(char* plaintext_key, char* key, char* plaintext){

    strcpy(plaintext_key, key);
    strcat(plaintext_key, "|");
    strcat(plaintext_key, plaintext);
    strcat(plaintext_key, "\0");

}
/***********************************************************************************************************
 * This function checks to see if there are any invalid characters in the key or plaintext files provided by the user.
 * The allowed characters are the capital letters and the space character. 
 * 
 * parameters: the contents of the key file,  the content of the plaintext file
****************************************************************************************************************************/
void check_invalid_characters(char* key, char* plaintext){

    int i = 0;
    for (i; i< strlen(plaintext)- 1; i++){ // if any of the characters in either file are invalid
        if (plaintext[i] > 90 || (plaintext[i] < 65 && plaintext[i] != ' '))
            error("ERROR invalid character in plaintext", 1);
...
int main(int argc, char *argv[]){
    ...
    char buffer[256];
    char key[256] = "\0"; 
    char plaintext[257] = "\0";
    char plaintext_key [515] = "\0";
    char okmessage[4] = "\0";
    ...
    if (strlen(plaintext) > 255)
        error("Plaintext is over 255 characters!", 2);

    store_plaintext_content(plaintext, argv[1]);

    store_key_content(key, argv[2]);



    if (strlen(key) < strlen(plaintext)){ // the key cannot be smaller than the plaintext file, otherwise there would be unencrypted characters.
        error("ERROR keysize too small", 1);
    }

    check_invalid_characters(key, plaintext);

    combine_key_plaintext(plaintext_key, key, plaintext);
    ...
    send(socketFD, "thisenc_client", strlen("thisenc_client"), 0); // send the message "thisenc_client" to let server know that this program is enc_client, not dec_client
    ...
    if (strcmp("NO", okmessage) == 0){ //If the message receive is NO. Only will happen if connected to dec_server.
        error("CLIENT: enc client cannot connect to dec server!", 1);
    }



    charsWritten = send(socketFD, plaintext_key, strlen(plaintext_key), 0); // Write to the server the plaintext|key
    ...
    charsRead = recv(socketFD, buffer, sizeof(buffer) - 1, 0); // The server sends back the cyphertext 
    
    if (charsRead < 0) 
        error("CLIENT: ERROR reading from socket", 2);

    printf("%s\n", buffer); // print the cyphertext to stdout
